    ...
    while let Some(event) = capture.next()? {
        if !setting.quiet {
            match event {
                ...
                event_capture::types::CaptureEvent::Parse(event) => print_parse_event(&event, &setting),
            }
        }
    }
...
fn print_scan_event(token: &event_capture::types::Token, setting: &CaptureSetting) {
    for (i, event) in token.leading_trivia.iter().enumerate() { 
        ...
        println!("     (kind) {}, (offset) {}, (len) {}", event.kind.name, event.offset, event.len);
...
fn print_parse_event(event: &event_capture::types::ParseEvent, setting: &CaptureSetting) {
    ...
    match event {
        ParseEvent::Shift(TransitionState{ kind, current, next, edit }) => {
            ...
            println!("    (state) current: {}, next: {}, edit: {}", current.unwrap(), next.unwrap(), edit);
        }
        ...
        ParseEvent::Invalid(TransitionState{ kind, current, edit, .. }) => {
            ...
            println!("     (kind) {}", kind.name);
            println!("    (state) current: {}, edit: {}", current.unwrap(), edit);
        }
